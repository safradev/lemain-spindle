...
usage() {
  cat <<'EOF'
Lemain Spindle · Linux

Uso:
  ./run-scripts/linux/spindle.sh              # verifica deps, instala se preciso e sobe a UI
  ./run-scripts/linux/spindle.sh ui           # sobe a UI Electron
  ./run-scripts/linux/spindle.sh cli <url> …  # CLI de download
  ./run-scripts/linux/spindle.sh setup        # só verifica/instala dependências
...
ensure_deps() {
  echo "==> Lemain Spindle · Linux · checando dependências"
  echo "    raiz: $ROOT"
...
case "$COMMAND" in
  ...
  setup)
    ensure_deps
    echo
    echo "Pronto. Para a UI: ./run-scripts/linux/spindle.sh ui"
    echo "Para a CLI: ./run-scripts/linux/spindle.sh cli <url>"
    ;;
  ui|dev|"")
    run_ui
    ;;
